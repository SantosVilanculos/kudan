...
from PySide6.QtGui import QFont, QHideEvent, QInputDevice, QShowEvent, QTextDocument
...
class Widget(QWidget):
    ...
    def q_timer_timeout(self) -> None:
        ...
        q_text_document.setPlainText(
            f"{str([f"(system_id={device.systemId()},name={device.name()},type={device.type()},capabilities={device.capabilities()})" for device in QInputDevice.devices()])}"
        )
