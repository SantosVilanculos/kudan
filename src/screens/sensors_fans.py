...
class List(QWidget):
    def __init__(self, name: str) -> None:
        ...
        for sfan in psutil.sensors_fans().get(self.name, list()):
            self.q_v_box_layout.addWidget(ListItem(sfan))

    def update(self) -> None:
        _A = psutil.sensors_fans().get(self.name, list())
        _B: list[str] = [sfan.label for sfan in _A]
        _C: list[str] = list()
        ...
        for index in range(self.q_v_box_layout.count()):
            ...
            if q_widget.sfan.label not in _B:
                self.q_v_box_layout.removeWidget(q_widget)
                q_widget.deleteLater()
            else:
                _C.append(q_widget.sfan.label)
                sfan = next(
                    (sfan for sfan in _A if sfan.label == q_widget.sfan.label),
                    None,
                )
                if sfan:
                    q_widget.update(sfan)

        for sfan in _A:
            if sfan.label not in _C:
                self.q_v_box_layout.addWidget(ListItem(sfan))
